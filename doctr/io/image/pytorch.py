...
__all__ = ["tensor_from_pil", "read_img_as_tensor", "read_img_as_tensor_grey", "decode_img_as_tensor", "decode_img_as_tensor_grey", "tensor_from_numpy", "get_img_shape"]
...
def tensor_from_pil_grey(pil_img: Image.Image, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    """Convert a PIL Image to a PyTorch tensor
    ...
    -------
        decoded image as tensor
    """
    img = tensor_from_numpy_grey(np.array(pil_img, np.uint8, copy=True), dtype)
...
def read_img_as_tensor_grey(img_path: AbstractPath, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    """Read an image file as a PyTorch tensor
    ...
    with Image.open(img_path, mode="r") as pil_img:
        image_converted = pil_img.convert('L')
        converted_tensor = tensor_from_pil(image_converted, dtype)
        #print(converted_tensor.shape)
        if converted_tensor.size()[0] == 3:
            print("warning conversion: ",  converted_tensor.shape)
...
def decode_img_as_tensor_grey(img_content: bytes, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    """Read a byte stream as a PyTorch tensor
    ...
    with Image.open(BytesIO(img_content), mode="r") as pil_img:
        return tensor_from_pil(pil_img.convert("L"), dtype)
...
def tensor_from_numpy_grey(npy_img: np.ndarray, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    """Read an image file as a PyTorch tensor
