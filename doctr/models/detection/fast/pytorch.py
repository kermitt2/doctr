...
from ...classification import textnet_base, textnet_small, textnet_tiny, textnet_grey
from ...modules.layers import FASTConvLayer
from ...utils import _bf16_to_float32, load_pretrained_params
from .base import _FAST, FASTPostProcessor

__all__ = ["FAST", "fast_tiny", "fast_small", "fast_base", "fast_grey", "reparameterize"]
...
class FASTGrey(_FAST, nn.Module):
    """
    FAST variant for grey images 1 channel
    ...
    def __init__(
        ...
        self.feat_extractor = self.feat_extractor.eval()
        with torch.no_grad():
            out = self.feat_extractor(torch.zeros((1, 1, 32, 32)))
            feat_out_channels = [v.shape[1] for _, v in out.items()]
...
def _fast_grey(
    ...
    **kwargs: Any,
    ...
    if not kwargs.get("class_names", None):
        ...
        kwargs["class_names"] = sorted(kwargs["class_names"])
    # Build the model
    model = FASTGrey(feat_extractor, cfg=default_cfgs[arch], **kwargs)
    # Load pretrained parameters
    """
    if pretrained:
        # The number of class_names is not the same as the number of classes in the pretrained model =>
...
def fast_grey(pretrained: bool = False, **kwargs: Any) -> FASTGrey:
    """
    Same as fast_base, but with 1 channel greyscale
    ...
    >>> from doctr.models import fast_base
    >>> model = fast_base(pretrained=True)
    >>> input_tensor = torch.rand((1, 1, 1024, 1024), dtype=torch.float32)
    >>> out = model(input_tensor)
