...
from doctr.io import read_img_as_tensor, tensor_from_numpy, read_img_as_tensor_grey
...
class AbstractDataset(_AbstractDataset):
    ...
    def _read_sample(self, index: int) -> Tuple[torch.Tensor, Any]:
        ...
        img = (
            ...
            else read_img_as_tensor_grey(os.path.join(self.root, img_name), dtype=torch.float32)
        )

        """
        img = (
            read_img_as_tensor_grey(os.path.join(self.root, img_name), dtype=torch.float32)
        )
        """
